/*
 * scroll-frames.js — scroll-scrubbed frame/video-sequence player for <canvas>.
 * Vendored from the MIT-licensed "roomwalk" plugin by Aironzak:
 * https://github.com/Aironzak/instagram  (plugins/roomwalk/skills/roomwalk/web/scroll_frames_standalone.js)
 *
 * No dependencies. Feed it your OWN frames (extract from any video) — Higgsfield is not required.
 *
 * Usage:
 *   const sf = new ScrollFrames({
 *     canvas: document.querySelector("#seq"),
 *     scroller: document.querySelector("#seq-section"), // tall element; progress = how far it has scrolled past
 *     dir: "assets/seq",                                // folder of frames
 *     meta: { frames: 600, pattern: "frame_%04d.jpg" },
 *     fit: "cover", zoom: 1, offsetX: 0, offsetY: 0,
 *     pingpong: false,                                  // play forward then reverse (for "exploded" single shots)
 *     timeline: [ {to:0.25, scroll:3}, {to:0.30, scroll:1}, {to:1, scroll:3} ], // pace: motion gets more scroll than stops
 *     onFrame: (p, slot) => {}                          // 0..1 progress, e.g. to cross-fade captions
 *   });
 *   sf.start();
 *
 * Section height rule of thumb: ~9-10px of scroll per frame.
 *   heightInScreens = (frames * 10 / window.innerHeight) + 1
 * Frames: aim for ~120 per 8s of source video (60 = visible stepping under a slow scrub).
 * Weight: 600 JPEG @864px ~= 32MB. Prefer WebP/AVIF, narrower frames upscaled by CSS,
 *   a sprite atlas, or scrub a real <video> (30s 1080p H.264 ~= 4MB; worse seek precision in Safari).
 */
